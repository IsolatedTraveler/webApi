...
<%  
        try{
			 ...
			 ExternalService externalService = (ExternalService) request.getAttribute("externalService");
			 Map<String,String> logMap = (HashMap)request.getAttribute("logMap");
			 ...
			 if(paramJson.containsKey("pageNumber")){//要 分页
				 ...
				 logMap.put("jssj", sdFull.format(new Date()));
				 logMap.put("dyjg", "1");
				 logMap.put("outdata", pageData.toString());
				 logMap.put("indata", data);
			 }else{
				 ...
				 logMap.put("jssj", sdFull.format(new Date()));
				 logMap.put("dyjg", "1");
				 logMap.put("outdata", resultJson.toString());
				 logMap.put("indata", data);
			 }
			 externalService.saveLog(logMap);
        }catch(Exception e){
        	ExternalService externalService = (ExternalService) request.getAttribute("externalService");
        	Map<String,String> logMap = (HashMap)request.getAttribute("logMap");
        	String data = (String) request.getAttribute("data");
        	...
        	logMap.put("dyjg", "0");
        	logMap.put("jssj", sdFull.format(new Date()));
			logMap.put("outdata", e.getMessage());
			logMap.put("indata", data);
			System.out.print(resultJson.toString());
			externalService.saveLog(logMap);
        }
