...
<%  
			 ...
			 try {
				 ...
				 String wjid = String.valueOf(paramJson.getString("wjid"));
				 String sql = "DELETE From EMR_WJJG  WHERE wjid=?";
				 ...
				 for(int i = 0 ; i < jgdatas.size();i++){
					 ...
					 if(StringUtils.isBlank(v_id)){
						 jgSql = "INSERT INTO EMR_WJJG (ID,wjid,DXXH,FDXXH,YSID,DXID,DXMC,DXNR,SFHH) VALUES (getseq('EMR_WJJG'),?,?,?,?,?,?,?,?)";
						 ...
						 jgSql = "INSERT INTO EMR_WJJG (ID,wjid,DXXH,FDXXH,YSID,DXID,DXMC,DXNR,SFHH) VALUES (?,?,?,?,?,?,?,?,?)";
						 st= connection.prepareStatement(jgSql);
				 ...
				 String nrdata = String.valueOf(paramJson.getString("nrdata"));
				 String blsql = "update EMR_WJGS set bbid=sys_guid(),nr=? where jlid=?";
				 st= connection.prepareStatement(blsql);
				 st.setObject(1, nrdata);
				 st.setObject(2, wjid);
				 int updateRows = st.executeUpdate();
				 if(updateRows == 0){
					 String gssql = "insert into  EMR_WJGS(bbid,nr,jlid) VALUES (sys_guid(),?,?)";
